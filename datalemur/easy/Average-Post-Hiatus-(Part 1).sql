...
-- 2. Use the HAVING clause with COUNT(post_id) > 1 to filter out users who have 
      only posted once.
      
-- 3. Calculate the time interval by subtracting the first post date (MIN) from 
      the latest post date (MAX).
      
-- 4. Use EXTRACT(DAY FROM ...) to convert the resulting time interval into a 
